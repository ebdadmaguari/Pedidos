...
// Função para salvar o pedido de forma completa
function saveCompleteOrder(pdfUrl) {
  ...
  const orderData = {
    ...
    pdfUrl: pdfUrl,
    details: getOrderDetails() // Captura todos os itens pedidos
  };

  // Obter pedidos existentes ou criar novo array
  let orders = [];
  try {
    const storedOrders = localStorage.getItem('revistaOrders');
  ...
  try {
    localStorage.setItem('revistaOrders', JSON.stringify(orders));
...
// Função para capturar todos os itens do pedido
function getOrderDetails() {
  ...
  rows.forEach(row => {
    ...
    const alunoInput = row.querySelector('td:nth-child(3) input');
    const professorInput = row.querySelector('td:nth-child(4) input');
    const kitInput = row.querySelector('td:nth-child(5) input');
    
    const aluno = alunoInput ? alunoInput.value || '0' : '0';
    const professor = professorInput ? professorInput.value || '0' : '0';
    const kit = kitInput ? kitInput.value || '0' : '0';
    const subtotal = row.cells[5]?.textContent.trim() || 'R$ 0,00';
...
// Modifique a função generateAndSharePDF
async function generateAndSharePDF() {
  // Verificar campos obrigatórios antes de prosseguir
  if (!validateForm()) {
  ...
  try {
    ...
    // Salvar o pedido completo
    const success = saveCompleteOrder(pdfUrl);
    
    if (success) {
      alertify.success('Pedido salvo com sucesso!');
      
      // Opção para ir para a área admin ou continuar
      alertify.confirm('Sucesso', 
        'Deseja acessar a área administrativa agora?',
        function() {
          window.location.href = 'admin.html';
        },
        function() {
          alertify.message('Você pode acessar a área admin depois');
        }
      );
    } else {
      alertify.error('Erro ao salvar pedido!');
    }
  } catch (error) {
    console.error("Erro no processo:", error);
    alertify.error('Ocorreu um erro ao processar seu pedido!');
  } finally {
    // Esconder loading
...
function validateForm() {
  ...
  if (!group || !congregation || !coordinator) {
    return false;
  }
